...
interface IAPIConsumer {
  ...
  function volume() external view returns (uint152);
}

contract ReputationAccessor {
  // Network: local
  // APIConsumer contract: 0xDc64a140Aa3E981100a9becA4E685f962f0cF6C9
  function requestReputationScore(address _api_consumer) public returns (bytes32) {
    return IAPIConsumer(_api_consumer).requestVolumeData();
  }

  function getReputationScore(address _api_consumer) public view returns (uint152) {
    return IAPIConsumer(_api_consumer).volume();
  }
}