...
interface IAPIConsumer {
    function requestReputationData(string memory, string memory) external returns (bytes32 requestId);
...
contract ReputationAccessor {
    ...
    function requestReputationScore(
        ...
        return IAPIConsumer(_api_consumer).requestReputationData(_query, _path);
    }
